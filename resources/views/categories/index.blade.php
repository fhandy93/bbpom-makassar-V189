    ...
    <div class="main-content app-content mt-0">
        <div class="side-app">
            ...
            <div class="main-container container-fluid">
                                    ...
                                    <table id="file-datatable" class="table table-bordered text-nowrap key-buttons border-bottom">
                                        <thead>
                                            <tr>
                                                ...
                                                <th class="border-bottom-0">Keyword</th>
                                        ...
                                        @foreach ($categories as $key => $item)
                                            ...
                                            <td>{{ $item->name }}</td>
                                            <td>{{ $item->slug }}</td>
                                            <td sty>{{ $item->keywords }}</td>
                                                <td>
                                                    <div class="btn-list">
                                                        <a href="/category/{{$item -> id}}/edit"><button type="button" class="btn btn-sm btn-primary"><span  class="fe fe-edit">Edit</span></button></a>
                                                        <form method="POST" action="{{ route('category.delete', $item->id) }}" style="display: inline;" >
                                                            @csrf
                                                            <input name="_method" type="hidden" value="DELETE">
                                                            <button type="submit" class="btn btn-sm btn-danger btn-flat show_confirm" data-toggle="tooltip" title='Delete' hidden><span class="fe fe-trash-2"></span></button>
                                                        </form>
                                                    </div>
                                                </td>
                                            </tr>
                ...
                <a href="/catinput"><button type="button" class="btn btn-primary"><i class="fe fe-plus me-2"></i>Tambah Category</button></a>
            </div>
        </div>
    </div>
    <!-- CONTAINER CLOSED -->
