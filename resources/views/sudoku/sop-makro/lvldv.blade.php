    ...
    <div class="main-content app-content mt-0">
        <div class="side-app">
            ...
            <div class="main-container container-fluid">
                ...
                <div class="page-header">
                    ...
                    <div>
                        <ol class="breadcrumb">
                            ...
                            <li class="breadcrumb-item active" aria-current="page">SOP Makro</li>
                        </ol>
                    </div>
                </div>
                <!-- PAGE-HEADER END -->
                                    ...
                                    <table id="file-datatable" class="table table-bordered text-nowrap key-buttons border-bottom">
                                        <thead>
                                            <tr>
                                                ...
                                                <th class="border-bottom-0">Kode SOP Makro</th>
                                                <th class="border-bottom-0">Judul SOP Makro</th>
                                        ...
                                        <tbody>
                                             @foreach($lvld as $index => $item)
                                            <tr>
                                             
                                                <td>{{$item -> kode_sop}}</td>
                                                <td>{{$item -> judul_sop }}</td>
                                                <td>
                                                    <form method="POST" action="/sop-makro-view/{{$item -> id}}"  >
                                                        @csrf
                                                ...
                                                <td>{{ $item->makroDet->map(fn($d) => \Carbon\Carbon::createFromFormat('d/m/Y', $d->tgl_terbit))->max()->format('d/m/Y') }}</td>
                                                <td>{{$item -> makroDet -> max('revisi') }}</td>
                                                <td>
                                                ...
                                                <td>
                                                    <div class="btn-list">
                                                        <a href="/sop-makro/{{$item -> id}}/detail"><button type="button" style="margin-right: 5px;" class="btn btn-sm btn-secondary" data-toggle="tooltip" title='Detail'><span  class="fe fe-align-justify"> Detail Revisi</span></button></a>&nbsp;
                                                        <a href="/edit-sop-makro/{{$item -> id}}"><button type="button" style="margin-right: 5px;" class="btn btn-sm btn-info" data-toggle="tooltip" title='Edit'><span  class="fe fe-edit"> Edit Data</span></button></a>&nbsp;
                                                        @if(checkPermission(['superadmin']))
                                                        <form method="POST" action="/sop-makro/{{ $item -> id }}/delete" style="display: inline;" >
                                                            @csrf
                ...
                <a href="/sop-makro-input/{{$proses_id}}"><button type="button" class="btn btn-primary"><i class="fe fe-plus me-2"></i>Tambah SOP Makro</button></a>
                @endif
                <p>
            </div>
        </div>
