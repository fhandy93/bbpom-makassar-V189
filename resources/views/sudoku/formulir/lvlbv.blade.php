        ...
        <div class="side-app">
            ...
            <div class="main-container container-fluid">
                ...
                <div class="page-header">
                    <h1 class="page-title">Data File Formulir Dokumen Pendukung</h1>
                    <div>
                        <ol class="breadcrumb">
                            <li class="breadcrumb-item"><a href="javascript:void(0)">SUDOKU</a></li>
                            <li class="breadcrumb-item active" aria-current="page">Formulir Dokumen Pendukung</li>
                        </ol>
                    </div>
                </div>
                <!-- PAGE-HEADER END -->
                    ...
                    <div class="col-lg-12">
                        <div class="card">
                            <div class="card-header">
                                <h3 class="card-title">Kode Induk: {{$lvla}}</h3>
                            </div>
                            <div class="card-body">
                                <div class="table-responsive">
                                    <table id="file-datatable" class="table table-bordered text-nowrap key-buttons border-bottom">
                                        <thead>
                                            <tr>
                                                <th class="border-bottom-0">Kode Formulir Dokumen Pendukung</th>
                                                <th class="border-bottom-0">Judul Formulir Dokumen Pendukung</th>
                                        ...
                                        <tbody>
                                            ...
                                            <tr>
                                                <td>{{$item -> formulir_kode}}</td>
                                                <td>{{$item -> judul_sop }}</td>
                                                <td>
                                                    <form method="POST" action="/formulir-view/{{$item -> id}}"  >
                                                        @csrf
                                                ...
                                                <td>{{$item -> formDet -> max('tgl_terbit') }}</td>
                                                <td>{{$item -> formDet -> max('revisi') }}</td>
                                                <td>
                                                    @if($item -> formDet->max('waktu')==1)
                                                    Aktif Berlaku
                                                    @elseif($item -> formDet->max('waktu')==2)
                                                    Aktif Tidak Berlaku
                                                    @else
                                                    Inaktif
                                                    @endif
                                                ...
                                                <td>
                                                    <div class="btn-list">
                                                        <a href="/formulir-revisi/{{$item -> id}}"><button type="button" style="margin-right: 5px;" class="btn btn-sm btn-secondary" data-toggle="tooltip" title='Detail'><span  class="fe fe-align-justify"> Detail Revisi</span></button></a>&nbsp;
                                                        <a href="/edit-formulir/{{$item -> id}}"><button type="button" style="margin-right: 5px;" class="btn btn-sm btn-info" data-toggle="tooltip" title='Edit'><span  class="fe fe-edit"> Edit Data</span></button></a>&nbsp;
                                                        @if(checkPermission(['superadmin','laboratorium']))
                                                        <form method="POST" action="/formulir/{{ $item->id }}/delete" style="display: inline;" >
                                                            @csrf
                ...
                @if(checkPermission(['superadmin','laboratorium']))
                <a href="/detail-formulir-input/{{$id}}"><button type="button" class="btn btn-primary"><i class="fe fe-plus me-2"></i>Tambah Formulir Dokumen Pendukung</button></a>
                @endif
                <p>
            </div>
        </div>
    ...
    setTimeout(function() {
        ...
        $('.show_confirm').click(function(event) {
            ...
            swal({
                ...
                text: "Menghapus data ini akan menghapus seluruh file dan data revisi didalamnya.",
                icon: "warning",
                buttons: true,
                dangerMode: true,
            })
