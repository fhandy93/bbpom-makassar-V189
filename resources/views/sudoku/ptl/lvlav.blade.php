        ...
        <div class="side-app">
            ...
            <div class="main-container container-fluid">
                ...
                <div class="page-header">
                    <h1 class="page-title">Data File Pedoman Teknis Laboratorium</h1>
                    <div>
                        <ol class="breadcrumb">
                            ...
                            <li class="breadcrumb-item active" aria-current="page">Pedoman Teknis Laboratorium</li>
                        </ol>
                    </div>
                </div>
                <!-- PAGE-HEADER END -->
                    ...
                    <div class="col-lg-12">
                        <div class="card">
                            <div class="card-header">
                                <h3 class="card-title">Pedoman Teknis Laboratorium</h3>
                            </div>
                            <div class="card-body">
                                <div class="table-responsive">
                                    <table id="file-datatable" class="table table-bordered text-nowrap key-buttons border-bottom">
                                        ...
                                        <tbody>
                                             @foreach($ptl as $index => $item)
                                            <tr>
                                                <td>{{$item -> ptl_kode}}</td>
                                                <td>{{$item -> judul }}</td>
                                                <td>
                                                    <form method="POST" action="/ptl-view/{{$item -> id}}"  >
                                                        @csrf
                                                ...
                                                <td>{{$item -> ptlDet -> max('tgl_terbit') }}</td>
                                                <td>{{$item -> ptlDet -> max('revisi') }}</td>
                                                <td>
                                                    @if($item -> ptlDet->max('waktu')==1)
                                                    Aktif Berlaku
                                                    @elseif($item -> ptlDet->max('waktu')==2)
                                                    Aktif Tidak Berlaku
                                                    @else
                                                    Inaktif
                                                    @endif
                                                ...
                                                <td>
                                                    <div class="btn-list">
                                                        <a href="/ptl-revisi/{{$item -> id}}"><button type="button" style="margin-right: 5px;" class="btn btn-sm btn-secondary" data-toggle="tooltip" title='Detail'><span  class="fe fe-align-justify"> Detail Revisi</span></button></a>&nbsp;
                                                        <a href="/edit-ptl/{{$item -> id}}"><button type="button" style="margin-right: 5px;" class="btn btn-sm btn-info" data-toggle="tooltip" title='Edit'><span  class="fe fe-edit"> Edit Data</span></button></a>&nbsp;
                                                        @if(checkPermission(['superadmin','laboratorium']))
                                                        <form method="POST" action="/ptl/{{ $item -> id }}/delete" style="display: inline;" >
                                                            @csrf
                ...
                @if(checkPermission(['superadmin','laboratorium']))
                <a href="/ptl-input"><button type="button" class="btn btn-primary"><i class="fe fe-plus me-2"></i>Tambah Pedoman Teknis Lab.</button></a>
                @endif
                <p>
            </div>
        </div>
