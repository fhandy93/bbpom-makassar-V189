        ...
        <div class="side-app">
            ...
            <div class="main-container container-fluid">
                ...
                <div class="page-header">
                    <h1 class="page-title">Detail Judul</h1>
                    <div>
                        <ol class="breadcrumb">
                            <li class="breadcrumb-item"><a href="javascript:void(0)">Level A</a></li>
                            <li class="breadcrumb-item active" aria-current="page">Detail Judul</li>
                        </ol>
                    </div>
                </div>
                <!-- PAGE-HEADER END -->
                                    ...
                                    <table id="file-datatable" class="table table-bordered text-nowrap key-buttons border-bottom">
                                        <thead>
                                            <tr>
                                                ...
                                                <th class="border-bottom-0">No Revisi</th>
                                                <th class="border-bottom-0">Jangka Waktu Penyimpanan</th>
                                        ...
                                        <tbody>
                                            ...
                                            <tr>
                                                <td>{{$index + 1}}</td>
                                                <td>{{$item -> klausul}}</td>
                                                <td>{{$item -> manualorg->judul}}</td> 
                                                <td>{{$item -> no_dokumen}}</td>
                                                <td>{{$item -> judul }}</td>
                                                <td>{{$item -> manOrgDet->max('tgl_terbit') }}</td>
                                                <td>{{$item -> manOrgDet->max('revisi')}}</td>
                                                <td>
                                                    @if($item -> manOrgDet->max('waktu')==1)
                                                    Aktif Berlaku
                                                    @elseif($item -> manOrgDet->max('waktu')==2)
                                                    Aktif Tidak Berlaku
                                                    @else
                                                    Inaktif
                                                    @endif
                                                ...
                                                <td>
                                                    <div class="btn-list">
                                                    <a href="/revisi-lvl-a/{{$data_id->id}}/{{$item -> id}}/1"><button type="button" style="margin-right: 5px;" class="btn btn-sm btn-secondary"><span  class="fe fe-align-justify"> Detail Revisi</span></button></a>&nbsp;
                                                        @if(checkPermission(['superadmin','laboratorium']))
                                                        <form method="POST" action="/man_org_judul/{{ $item -> id }}/delete" style="display: inline;" >
                                                            @csrf
                ...
                <a href="/detail-judul-input/{{$data_id->id}}"><button type="button" class="btn btn-primary"><i class="fe fe-plus me-2"></i>Tambah Judul</button></a>
                @endif
                <p>
            </div>
        </div>
