        ...
        <div class="side-app">
            ...
            <div class="main-container container-fluid">
                ...
                <div class="page-header">
                    <h1 class="page-title">Data Survey</h1>
                    <div>
                        <ol class="breadcrumb">
                            <li class="breadcrumb-item"><a href="javascript:void(0)">SIPINTAR</a></li>
                            <li class="breadcrumb-item active" aria-current="page">Survey View</li>
                        </ol>
                    </div>
                </div>
                <!-- PAGE-HEADER END -->
                <div class="row row-sm">
					...
					@endif 
                    <div class="col-lg-2">
                        <a href="/sipintar-v1" class="btn btn-primary"><span  class="fa fa-arrow-left"> Kembali</span></a>
                    </div>
                    <p>
                    <div class="col-lg-12">
						<div class="card">
							...
							<div class="card-body">
								<form id="signupForm" method="post" class="form-horizontal" action="/sipintar/data-survey/filter" >
									@csrf
									@method('post')		
                    ...
                    <div class="col-lg-12">
                        <div class="card">
                            <div class="card-header">
                                <h3 class="card-title">Data Survey 2024</h3>
                            </div>
                            <div class="card-body">
                                    ...
                                    <table id="file-datatable" class="table table-bordered text-nowrap key-buttons border-bottom">
                                        <thead>
                                            <tr>
                                                ...
                                                <th class="border-bottom-0">Nama Pengunjung</th>
                                                <th class="border-bottom-0">Petugas</th>
                                                <th class="border-bottom-0">Survey</th>
                                                <th class="border-bottom-0">Penampilan</th>
                                                <th class="border-bottom-0">Sikap</th>
                                                <th class="border-bottom-0">Kesigapan</th>
                                                <th class="border-bottom-0">Input Survey</th>
                                        ...
                                        @foreach($data as $index => $data)
                                            <tr>
                                                <td>{{$index + 1}}</td>
                                                <td ><a href="/sipintar/data-tamu/{{$data -> lay->id}}/detail" class="text-success">{{$data -> lay-> nama}}</a></td>
                                                <td>{{$data -> pet-> nama}}</td>   
                                                <td>
                                                     @switch($data -> skala_survey)
                                                        @case(1)
                                                            <span class="badge rounded-pill bg-danger">Kurang Puas</span>
                                                            @break

                                                        @case(2)
                                                            <span class="badge rounded-pill bg-warning">Cukup Puas</span>
                                                            @break

                                                        @case(3)
                                                            <span class="badge rounded-pill bg-primary">Puas</span>
                                                            @break

                                                        @default
                                                            <span class="badge rounded-pill bg-success">Sangat Memuaskan</span>
                                                    @endswitch
                                                </td>
                                                <td>{{$data -> tampilan}}</td>
                                                <td>{{$data -> sikap}}</td>
                                                <td>{{$data -> kesigapan}}</td>
                                                <td>{{$data -> created_at}}</td>
                                                <td>
                                                    <div class="btn-list">
                                                        <form method="POST" action="/sipintar/data-survey/{{$data->id}}/delete" style="display: inline;" >
                                                            @csrf
    ...
    setTimeout(function() {
        ...
        $('.show_confirm').click(function(event) {
            ...
            swal({
                ...
                text: "Jika anda hapus, Data akan hilang dari DATABASE.",
                icon: "warning",
                buttons: true,
                dangerMode: true,
            })
