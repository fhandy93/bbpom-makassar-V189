        ...
        <div class="side-app">
            ...
            <div class="main-container container-fluid">
                ...
                <div class="page-header">
                    <h1 class="page-title">Data Transaksi Alat Gelas Kuantitatif</h1>
                    <div>
                        <ol class="breadcrumb">
                            <li class="breadcrumb-item"><a href="javascript:void(0)">Alat Gelas Kuantitatif</a></li>
                            <li class="breadcrumb-item active" aria-current="page">Transaksi Alat Gelas Kuantitatif View</li>
                        </ol>
                    </div>
                </div>
                <!-- PAGE-HEADER END -->
                <div class="row row-sm">
                    ...
                    <div class="col-lg-12">
                        ...
                        <a href="/kuantitatiftrans/{{ $trans['id'] }}"><button type="button" style="width: 200px;" class="btn btn-primary"><span  class="fe fe-edit">  Edit Stok</span></button></a><p>
                        @endif
                        <div class="card">
                            <div class="card-header">
                                <h3 class="card-title">Transaksi Alat Gelas Kuantitatif</h3>
                            </div>
                                    ...
                                    <table id="file-datatable" class="table table-bordered text-nowrap key-buttons border-bottom">
                                        <thead>
                                            <tr>
                                                ...
                                                <th class="border-bottom-0">Nama Alat Gelas</th>
                                                <th class="border-bottom-0">Jenis Transaksi</th>
                                                <th class="border-bottom-0">Jumlah Stok</th>
                                                <th class="border-bottom-0">Tanggal Transaksi</th>
                                                <th class="border-bottom-0">User</th>
                                               
                                            </tr>
                                        </thead>
                                        <tbody>
                                             @foreach($trans['data'] as $index => $item)
                                            <tr>
                                                <td>{{$index + 1}}</td>
                                                <td>{{$item -> kuan-> nama }}</td>
                                                <td>
                                                    @if($item -> jenis=="Transaksi Masuk Kosmetik" 
                                                                 or $item -> jenis=="Transaksi Masuk Pangan" 
                                                                 or $item -> jenis=="Transaksi Masuk OT" 
                                                                 or $item -> jenis=="Transaksi Masuk Mikro" 
                                                                 or $item -> jenis=="Transaksi Masuk Obat")
                                                    {{$item -> jenis}}  <span  style="color:greenyellow;" class="fa fa-arrow-circle-down"></span>
                                                    @else
                                                    {{$item -> jenis}}  <span  style="color:red;" class="fa fa-arrow-circle-up"></span>
                                                    @endif
                                                </td>
                                                <td>{{$item -> stok}}</td>
