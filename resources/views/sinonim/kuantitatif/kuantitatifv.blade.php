<x-HomeLayout>
        ...
        <div class="side-app">
            ...
            <div class="main-container container-fluid">
                ...
                <div class="page-header">
                    <h1 class="page-title">Data Alat Gelas Kuantitatif</h1>
                    <div>
                        <ol class="breadcrumb">
                            <li class="breadcrumb-item"><a href="javascript:void(0)">Sinonim</a></li>
                            <li class="breadcrumb-item active" aria-current="page">Alat Gelas Kuantitatif View</li>
                        </ol>
                    </div>
                </div>
                <!-- PAGE-HEADER END -->
                    ...
                    <div class="col-lg-12">
                        <div class="card">
                            <div class="card-header">
                                <h3 class="card-title"> Alat Gelas Kuantitatif</h3>
                            </div>
                            <div class="card-body">
                                    ...
                                    <table id="file-datatable" class="table table-bordered text-nowrap key-buttons border-bottom">
                                        <thead>
                                            <tr>
                                                ...
                                                <th class="border-bottom-0">Nama Alat Gelas Kuantitatif</th>
                                        ...
                                        <tbody>
                                            ...
                                            <tr>
                                                ...
                                                <td>{{$kuan -> merek}}</td>
                                                <td>{{$kuan -> ukuran}}</td>  
                                                <td @if($kuan -> kosmetik <= 0) style="color: red;"@endif>{{$kuan -> kosmetik}}</td>
                                                <td @if($kuan -> pangan <= 0) style="color: red;"@endif>{{$kuan -> pangan}}</td>
                                                <td @if($kuan -> ot <= 0) style="color: red;"@endif>{{$kuan -> ot}}</td>
                                                <td @if($kuan -> mikro <= 0) style="color: red;"@endif>{{$kuan -> mikro}}</td>
                                                <td @if($kuan -> obat <= 0) style="color: red;"@endif>{{$kuan -> obat}}</td>           
                                                <td @if(($kuan -> kosmetik + $kuan -> pangan + $kuan -> ot + $kuan -> mikro + $kuan -> obat) <= 0) style="color: red;"@endif><b>{{$kuan -> kosmetik + $kuan -> pangan + $kuan -> ot + $kuan -> mikro + $kuan -> obat}}</b></td>
                                                @if(checkPermission(['superadmin','perlengkapan','laboratorium']))
                                                <td>
                                                    <div class="btn-list">
                                                    <a href="/kuantitatif/{{$kuan -> id}}/detail"><button type="button" style="margin-right: 5px;" class="btn btn-sm btn-secondary"><span  class="fe fe-eye">Detail</span></button></a>&nbsp;
                                                    <a href="/kuantitatif/{{$kuan -> id}}/edit"><button type="button" style="margin-right: 5px;" class="btn btn-sm btn-primary"><span  class="fe fe-edit">Edit</span></button></a>
                                                        <form method="POST" action="kuantitatifd/{{ $kuan -> id }}" style="display: inline;" >
                                                            @csrf
                ...
                @if(checkPermission(['superadmin','perlengkapan','laboratorium']))
                <a href="/kuantitatif"><button type="button" class="btn btn-primary"><i class="fe fe-plus me-2"></i>Tambah Alat Gelas Kuantitatif</button></a>
                <a href="/kuan_formc"><button type="button" class="btn btn-success">Cetak Laporan Akhir Bulanan</button></a>
                @endif
                @if(checkPermission(['superadmin']))
                <a href="/rekap_kuan"><button type="button" class="btn btn-warning show_confirm2">Input Stok Akhir Bulanan</button></a>
                @endif<p>
            </div>
        </div>
    </div>
        ...
        <div class="modal-dialog modal-dialog-centered text-center" role="document">
            <div class="modal-content modal-content-demo">
                <div class="modal-header">
                    <h6 class="modal-title">Petunjuk Stok Alat Gelas Kuantitatif</h6><button aria-label="Close" class="btn-close" data-bs-dismiss="modal"><span aria-hidden="true">&times;</span></button>
                </div>
                <div class="modal-body">
                    <p>Tuliskan Nama Alat Gelas Kuantitatif didalam kolom pencarian (Search box) untuk mempermudah menemukan Stok Alat Gelas Kuantitatif yang dicari</p>
                </div>
    ...
    setTimeout(function() {
        ...
        $('.show_confirm2').click(function(event) {
            ...
            event.preventDefault();
            swal({
                title: `Anda yakin ingin menginput STOK AKHIR BULAN Alat Gelas Kuantitatif ?`,
