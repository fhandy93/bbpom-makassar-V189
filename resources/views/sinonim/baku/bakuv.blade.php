<x-HomeLayout>
        ...
        <div class="side-app">
            ...
            <div class="main-container container-fluid">
                ...
                <div class="page-header">
                    <h1 class="page-title">Data Baku Pembanding</h1>
                    <div>
                        <ol class="breadcrumb">
                            <li class="breadcrumb-item"><a href="javascript:void(0)">Sinonim</a></li>
                            <li class="breadcrumb-item active" aria-current="page">Baku Pembanding View</li>
                        </ol>
                    </div>
                </div>
                <!-- PAGE-HEADER END -->
                                    ...
                                    <table id="file-datatable" class="table table-bordered text-nowrap key-buttons border-bottom">
                                        <thead>
                                            <tr>
                                                ...
                                                <th class="border-bottom-0">Nama Baku Pembanding</th>
                                                <th class="border-bottom-0">No. Kontrol</th>
                                                <th class="border-bottom-0">Vial</th>
                                        ...
                                        <tbody>
                                             @foreach($baku as $index => $baku)
                                            <tr>
                                                <td>{{$index + 1}}</td>
                                                <td>{{$baku -> nama}}</td>
                                                <td>{{$baku -> no_kontrol }}</td>
                                                <td>{{$baku -> bobot }}</td>   
                                                <td @if($baku -> kosmetik <= 0) style="color: red;"@endif>{{$baku -> kosmetik}}</td>
                                                <td @if($baku -> pangan <= 0) style="color: red;"@endif>{{$baku -> pangan}}</td>
                                                <td @if($baku -> ot <= 0) style="color: red;"@endif>{{$baku -> ot}}</td>
                                                <td @if($baku -> mikro <= 0) style="color: red;"@endif>{{$baku -> mikro}}</td>
                                                <td @if($baku -> obat <= 0) style="color: red;"@endif>{{$baku -> obat}}</td>           
                                                <td @if(($baku -> kosmetik + $baku -> pangan + $baku -> ot + $baku -> mikro + $baku -> obat) <= 0) style="color: red;"@endif><b>{{$baku -> kosmetik + $baku -> pangan + $baku -> ot + $baku -> mikro + $baku -> obat}}</b></td>
                                                @if(checkPermission(['superadmin','perlengkapan','laboratorium']))
                                                <td>
                                                    <div class="btn-list">
                                                    <a href="/baku/{{$baku -> id}}/detail"><button type="button" style="margin-right: 5px;" class="btn btn-sm btn-secondary"><span  class="fe fe-eye">Detail</span></button></a>&nbsp;
                                                    <a href="/baku/{{$baku -> id}}/edit"><button type="button" style="margin-right: 5px;" class="btn btn-sm btn-primary"><span  class="fe fe-edit">Edit</span></button></a>
                                                        <form method="POST" action="bakud/{{ $baku -> id }}" style="display: inline;" >
                                                            @csrf
                ...
                @if(checkPermission(['superadmin','perlengkapan','laboratorium']))
                <a href="/baku"><button type="button" class="btn btn-primary"><i class="fe fe-plus me-2"></i>Tambah Baku Pembanding</button></a>
                <a href="/baku_formc"><button type="button" class="btn btn-success">Cetak Laporan Akhir Bulanan</button></a>
                @endif
                @if(checkPermission(['superadmin']))
                <a href="/rekap_baku"><button type="button" class="btn btn-warning show_confirm2">Input Stok Akhir Bulanan</button></a>
                @endif<p>
                <p>
            </div>
        </div>
        ...
        <div class="modal-dialog modal-dialog-centered text-center" role="document">
            <div class="modal-content modal-content-demo">
                <div class="modal-header">
                    <h6 class="modal-title">Petunjuk Stok Baku Pembanding</h6><button aria-label="Close" class="btn-close" data-bs-dismiss="modal"><span aria-hidden="true">&times;</span></button>
                </div>
                <div class="modal-body">
                    <p>Tuliskan Nama Baku Pembanding didalam kolom pencarian (Search box) untuk mempermudah menemukan Stok Baku Pembanding yang dicari</p>
                </div>
    ...
    setTimeout(function() {
        ...
        $('.show_confirm2').click(function(event) {
            ...
            event.preventDefault();
            swal({
                title: `Anda yakin ingin menginput STOK AKHIR BULAN Baku Pembanding ?`,
